...
import shutil
import time
from utils import get_coregistered_atlas_native_space
from nilearn import plotting


class SubjectSpaceAtlas:
    def __init__(self):
        self.rootDirPath = pathlib.Path(r'A:\data\bids')
        # Import the subjectList from the csv file
        self.subjectList = ["sub-resi001", "sub-resi002", "sub-resi003", "sub-resi004",
    ...
    def create_atlas(self):
        ...
        # Make the initial bids native folder(needs to be done only once)
        rootDir = self.rootDirPath / "bids_native_final"
        if rootDir.exists():
            try:
                ...
                print("Nothing exists inside the root folder")
                rootDir.rmdir()
                rootDir.mkdir()
            # rootDir = rootDirPath / "bids_native_space_" / timestr
        else:
            rootDir.mkdir()

        # Loop over all the subjects from the subjectList
        for subject in self.subjectList:
            # Find the path for the t1_file
            t1_file = pathlib.Path(self.rootDirPath, subject, "ses-1", "anat",
                                   (subject + "_ses-1_T1w.nii"))

            # Find the path for the vwbs_file
            vwbs_file = pathlib.Path(self.rootDirPath, subject, "ses-1", "anat",
                                     (subject + "_ses-1_T1w_vwbs.nii"))

            # Find the path for hte vwbs_text_file
            vwbs_file_txt = pathlib.Path(self.rootDirPath, subject, "ses-1", "anat",
                                         (subject + "_ses-1_T1w_vwbs_atlas.txt"))

            # DWI folder for copying
            dwi_folder = pathlib.Path(self.rootDirPath, subject, "ses-1", "dwi")

            # Find the path for the BOLD nii func file
            bold_func_file_nii = pathlib.Path(self.rootDirPath, subject, "ses-1", "func",
                                              (subject + "_ses-1_task-rest_bold.nii"))

            # Find the path for the BOLD json func file
            bold_func_file_json = pathlib.Path(self.rootDirPath, subject, "ses-1", "func",
                                               (subject + "_ses-1_task-rest_bold.json"))

            # Find the path for the hires nii func file
            hires_func_file_nii = pathlib.Path(self.rootDirPath, subject, "ses-1", "func",
                                               (subject + "_ses-1_task-rest_hires.nii"))

            # If both the files exist, run the coregistering function else
            # print out an error
            if t1_file.exists() and vwbs_file.exists():
                get_coregistered_atlas_native_space(rootDir,
                                                    t1_file,
                                                    vwbs_file,
                                                    vwbs_file_txt,
                                                    dwi_folder,
    ...
    def visualize_atlas(self):
        for subject in self.subjectList:
            mean_func_file_path = pathlib.Path(self.rootDirPath / 'bids_native_final' / subject / 'ses-1' / 'func'
                                               / ('art_mean_au' + subject + '_ses-1_task-rest_bold.nii'))

            vwbs_file_path = pathlib.Path(self.rootDirPath / 'bids_native_final' / subject / 'ses-1' / 'anat'
